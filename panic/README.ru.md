...
- **«Мгновенно» никто не обещает — запуск измеряется.** Шаги здесь — внешние команды
  (`hdiutil detach`, блокировка экрана), общей гарантии по времени у них нет: занятый том,
  зависшая системная команда или отсутствующий механизм блокировки займут столько, сколько
  займут. Поэтому `panic now` печатает замеренную длительность двух вещей этого запуска —
  отсоединения образов и шага блокировки — и отдельной строкой говорит, удалась ли сама
  блокировка.
- **Порядок выбран осознанно: сначала тома, потом экран.** Запертый экран над примонтированным
  сейфом не защищает ни от кого, кто унесёт машину; закрытый сейф переживёт обход блокировки.
  Цена — экран остаётся видимым, пока идёт отсоединение, и именно эта длительность теперь
  печатается.
